/**
 * @type {Record<Internal.EntityType_<Internal.LivingEntity>, number[]>}
 * Numbers are as follow:
 * - Max Health
 * - Armor
 * - Attack Damage
 */
global.hpModifications = {
	"minecraft:allay": [100, 10, 0],
	"minecraft:armor_stand": [100, 15, 0],
	"minecraft:axolotl": [70, 10, 0],
	"minecraft:bat": [20, 15, 0],
	"minecraft:bee": [50, 10, 0],
	"minecraft:blaze": [80, 30, 0],
	"minecraft:camel": [160, 15, 0],
	"minecraft:cat": [50, 15, 0],
	"minecraft:cave_spider": [60, 10, 0],
	"minecraft:chicken": [20, 15, 0],
	"minecraft:cod": [15, 15, 0],
	"minecraft:cow": [50, 15, 0],
	"minecraft:creeper": [100, 10, 0],
	"minecraft:dolphin": [50, 15, 0],
	"minecraft:donkey": [265, 15, 0],
	"minecraft:drowned": [100, 15, 2],
	"minecraft:elder_guardian": [400, 40, 0],
	"minecraft:ender_dragon": [15000, 15, 0],
	"minecraft:enderman": [200, 35, 0],
	"minecraft:endermite": [40, 10, 0],
	"minecraft:evoker": [120, 10, 0],
	"minecraft:fox": [50, 10, 0],
	"minecraft:frog": [50, 50, 0],
	"minecraft:ghast": [50, 15, 0],
	"minecraft:giant": [500, 250, 0],
	"minecraft:glow_squid": [50, 15, 0],
	"minecraft:goat": [50, 10, 0],
	"minecraft:guardian": [150, 30, 0],
	"minecraft:hoglin": [200, 30, 0],
	"minecraft:horse": [265, 15, 0],
	"minecraft:husk": [110, 15, 2],
	"minecraft:illusioner": [160, 10, 0],
	"minecraft:iron_golem": [1500, 75, 0],
	"minecraft:llama": [265, 15, 0],
	"minecraft:magma_cube": [5, 5, 3],
	"minecraft:mooshroom": [50, 15, 0],
	"minecraft:mule": [265, 15, 0],
	"minecraft:ocelot": [50, 15, 0],
	"minecraft:panda": [100, 30, 0],
	"minecraft:parrot": [30, 15, 0],
	"minecraft:phantom": [50, 10, 0],
	"minecraft:pig": [50, 15, 0],
	"minecraft:piglin": [80, 25, 0],
	"minecraft:piglin_brute": [250, 35, 0],
	"minecraft:pillager": [120, 25, 0],
	"minecraft:polar_bear": [150, 30, 0],
	"minecraft:pufferfish": [15, 15, 0],
	"minecraft:rabbit": [30, 15, 0],
	"minecraft:ravager": [500, 60, 0],
	"minecraft:salmon": [15, 15, 0],
	"minecraft:sheep": [40, 15, 0],
	"minecraft:shulker": [150, 15, 0],
	"minecraft:silverfish": [40, 5, 0],
	"minecraft:skeleton": [100, 10, 0],
	"minecraft:skeleton_horse": [75, 15, 0],
	"minecraft:slime": [5, 5, 0],
	"minecraft:sniffer": [70, 15, 0],
	"minecraft:snow_golem": [20, 15, 0],
	"minecraft:spider": [80, 10, 0],
	"minecraft:squid": [50, 15, 0],
	"minecraft:stray": [80, 10, 0],
	"minecraft:strider": [100, 15, 0],
	"minecraft:tadpole": [30, 15, 0],
	"minecraft:trader_llama": [265, 15, 0],
	"minecraft:tropical_fish": [15, 15, 0],
	"minecraft:turtle": [150, 15, 0],
	"minecraft:vex": [70, 20, 0],
	"minecraft:villager": [100, 15, 0],
	"minecraft:vindicator": [120, 25, 0],
	"minecraft:wandering_trader": [100, 15, 0],
	"minecraft:warden": [2500, 1000, 0],
	"minecraft:witch": [130, 10, 0],
	"minecraft:wither": [7500, 10, 4],
	"minecraft:wither_skeleton": [100, 10, 0],
	"minecraft:wolf": [40, 20, 0],
	"minecraft:zoglin": [200, 30, 0],
	"minecraft:zombie": [100, 15, 2],
	"minecraft:zombie_horse": [75, 15, 0],
	"minecraft:zombie_villager": [100, 15, 2],
	"minecraftzombified_piglin": [100, 25, 2],
	"majruszsdifficulty:cerberus": [1200, 40, 0],
	"majruszsdifficulty:creeperling": [30, 10, 0],
	"majruszsdifficulty:giant": [1000, 50, 0],
	"majruszsdifficulty:illusioner": [160, 10, 0],
	"majruszsdifficulty:tank": [700, 30, 0],
	"botania:pixie": [10, 15, 0],
	"botania:doppleganger": [1600, 15, 0],
	"botania:pink_wither": [1500, 10, 4],
	"the_bumblezone:variant_bee": [50, 10, 0],
	"the_bumblezone:honey_slime": [40, 5, 0],
	"the_bumblezone:beehemoth": [200, 20, 0],
	"the_bumblezone:bee_queen": [750, 50, 0],
	"the_bumblezone:rootmin": [50, 15, 0],
	"the_bumblezone:cosmic_crystal_entity": [300, 5, 0],
	"mutantmonsters:creeper_minion": [20, 15, 0],
	"mutantmonsters:mutant_creeper": [600, 25, 0],
	"mutantmonsters:mutant_enderman": [1000, 35, 0],
	"mutantmonsters:mutant_skeleton": [750, 15, 0],
	"mutantmonsters:mutant_snow_golem": [400, 15, 0],
	"mutantmonsters:mutant_zombie": [750, 60, 0],
	"mutantmonsters:spider_pig": [200, 15, 0],
	"mutantmonsters:endersoul_clone": [5, 5, 0],
	"phantasm:crystie": [60, 10, 0],
	"phantasm:behemoth": [300, 75, 0],
	"illagerinvasion:provoker": [120, 10, 0],
	"illagerinvasion:invoker": [1250, 40, 0],
	"illagerinvasion:necromancer": [160, 10, 0],
	"illagerinvasion:basher": [160, 15, 0],
	"illagerinvasion:sorcerer": [160, 10, 0],
	"illagerinvasion:archivist": [120, 10, 0],
	"illagerinvasion:inquisitor": [400, 50, 0],
	"illagerinvasion:marauder": [120, 10, 0],
	"illagerinvasion:alchemist": [120, 10, 0],
	"illagerinvasion:firecaller": [160, 10, 0],
	"illagerinvasion:surrendered": [70, 25, 0],
	"galosphere:sparkle": [50, 15, 0],
	"galosphere:spectre": [20, 10, 0],
	"galosphere:specterpillar": [10, 15, 0],
	"galosphere:spectator_vision": [5, 15, 0],
	"galosphere:berserker": [775, 50, 0],
	"galosphere:preserved": [100, 20, 2],
	"spelunkers_charm:tremor_handler": [500, 15, 0],
	"spelunkers_charm:sound_entity": [50, 15, 0],
	"born_in_chaos_v5:decrepit_skeleton": [75, 3, 0],
	"born_in_chaos_v5:skeleton_demoman": [125, 3, 2],
	"born_in_chaos_v5:decaying_zombie": [125, 4, 0],
	"born_in_chaos_v5:baby_skeleton": [50, 2, 1],
	"born_in_chaos_v5:baby_skeleton_minion": [50, 2, 1],
	"born_in_chaos_v5:controlled_baby_skeleton": [100, 4, 1],
	"born_in_chaos_v5:barrel_zombie": [150, 3, 6],
	"born_in_chaos_v5:phantom_creeper": [100, 3, 0],
	"born_in_chaos_v5:phantom_creeper_copy": [100, 3, 4],
	"born_in_chaos_v5:door_knight": [150, 6, 7],
	"born_in_chaos_v5:restless_spirit": [120, 2, 0],
	"born_in_chaos_v5:skeleton_thrasher": [250, 8, 0],
	"born_in_chaos_v5:pumpkin_spirit": [330, 13, 2],
	"born_in_chaos_v5:seared_spirit": [250, 10, 3],
	"born_in_chaos_v5:firelight": [75, 2, 0],
	"born_in_chaos_v5:diamond_termite": [70, 2, 8],
	"born_in_chaos_v5:bonescaller": [125, 3, 1],
	"born_in_chaos_v5:bone_imp": [100, 3, 3],
	"born_in_chaos_v5:bone_imp_minion": [100, 3, 3],
	"born_in_chaos_v5:supreme_bonescaller": [325, 5, 5],
	"born_in_chaos_v5:supreme_bonescaller_stage_10": [60, 5, 5],
	"born_in_chaos_v5:dark_vortex": [175, 4, 0],
	"born_in_chaos_v5:dread_hound": [85, 5, 0],
	"born_in_chaos_v5:fallen_chaos_knight": [600, 5, 2],
	"born_in_chaos_v5:scarlet_persecutor": [175, 5, 0],
	"born_in_chaos_v5:spirit_guide": [160, 3, 3],
	"born_in_chaos_v5:spirit_guide_assistant": [75, 2, 3],
	"born_in_chaos_v5:zombie_clown": [175, 3, 4],
	"born_in_chaos_v5:mr_pumpkin": [60, 3, 0],
	"born_in_chaos_v5:mr_pumpkin_controlled": [130, 3, 0],
	"born_in_chaos_v5:sir_pumpkinhead": [800, 6, 9],
	"born_in_chaos_v5:felsteed": [300, 3, 2],
	"born_in_chaos_v5:sir_pumpkinhead_without_horse": [600, 3, 4],
	"born_in_chaos_v5:sir_the_headless": [350, 4, 4],
	"born_in_chaos_v5:pumpkinhead": [325, 3, 4],
	"born_in_chaos_v5:riding_felsteed": [300, 3, 0],
	"born_in_chaos_v5:decaying_zombie_not_despawn": [150, 4, 0],
	"born_in_chaos_v5:door_knight_not_despawn": [150, 6, 7],
	"born_in_chaos_v5:bonescaller_not_despawn": [125, 3, 0],
	"born_in_chaos_v5:supreme_bonescaller_not_despawn": [325, 5, 5],
	"born_in_chaos_v5:skeleton_thrasher_not_despawn": [250, 8, 8],
	"born_in_chaos_v5:dread_hound_not_despawn": [85, 5, 0],
	"born_in_chaos_v5:firelight_not_despawn": [75, 2, 0],
	"born_in_chaos_v5:zombie_clown_not_despawn": [300, 5, 4],
	"born_in_chaos_v5:seared_spirit_not_despawn": [250, 10, 3],
	"born_in_chaos_v5:dire_hound_leader": [500, 10, 0],
	"born_in_chaos_v5:maggot": [5, 1, 0],
	"born_in_chaos_v5:corpse_fly": [50, 2, 0],
	"born_in_chaos_v5:bloody_gadfly": [100, 3, 0],
	"born_in_chaos_v5:corpse_fish": [60, 2, 0],
	"born_in_chaos_v5:zombie_fisherman": [125, 4, 4],
	"born_in_chaos_v5:swarmer": [200, 4, 4],
	"born_in_chaos_v5:thornshell_crab": [125, 4, 4],
	"born_in_chaos_v5:glutton_fish": [450, 12, 3],
	"born_in_chaos_v5:zombie_bruiser": [300, 6, 2],
	"born_in_chaos_v5:pumpkin_bomb": [75, 1, 0],
	"born_in_chaos_v5:senor_pumpkin": [175, 5, 2],
	"born_in_chaos_v5:lord_pumpkinhead": [3330, 12, 6],
	"born_in_chaos_v5:lords_felsteed": [1300, 9, 6],
	"born_in_chaos_v5:lord_pumpkinhead_withouta_horse": [1800, 6, 2],
	"born_in_chaos_v5:lord_the_headless": [1500, 12, 2],
	"born_in_chaos_v5:lord_pumpkinhead_head": [1000, 6, 9],
	"born_in_chaos_v5:infernal_spirit": [450, 16, 2],
	"born_in_chaos_v5:riding_lords_felsteed": [450, 9, 2],
	"born_in_chaos_v5:zombie_lumberjack": [125, 9, 4],
	"born_in_chaos_v5:siamese_skeletons": [100, 3, 2],
	"born_in_chaos_v5:siamese_skeletonsleft": [50, 2, 1],
	"born_in_chaos_v5:siamese_skeletonsright": [50, 2, 1],
	"born_in_chaos_v5:missioner": [750, 9, 0],
	"born_in_chaos_v5:phantom_bomb_entity": [75, 1, 0],
	"born_in_chaos_v5:controlled_spiritual_assistant": [150, 6, 3],
	"born_in_chaos_v5:spiritof_chaos": [75, 3, 0],
	"born_in_chaos_v5:lifestealer": [500, 7, 6],
	"born_in_chaos_v5:lifestealer_true_form": [500, 7, 5],
	"born_in_chaos_v5:baby_spider": [50, 2, 0],
	"born_in_chaos_v5:mother_spider": [450, 6, 0],
	"born_in_chaos_v5:baby_spider_controlled": [50, 2, 0],
	"born_in_chaos_v5:nightmare_stalker": [200, 7, 4],
	"born_in_chaos_v5:pumpkin_dunce": [100, 3, 0],
	"born_in_chaos_v5:mrs_pumpkin": [150, 3, 1],
	"born_in_chaos_v5:pumpkin_bruiser": [450, 9, 6],
	"born_in_chaos_v5:krampus": [1250, 14, 0],
	"born_in_chaos_v5:krampus_henchman": [150, 4, 5],
	"born_in_chaos_v5:missionary_raider": [750, 9, 0],
	"cataclysm:ender_golem": [750, 50, 2],
	"cataclysm:ender_guardian": [1665, 80, 0],
	"cataclysm:old_netherite_monstrosity": [2500, 105, 0],
	"cataclysm:netherite_monstrosity": [3000, 125, 2],
	"cataclysm:netherite_ministrosity": [600, 15, 5],
	"cataclysm:nameless_sorcerer": [250, 10, 0],
	"cataclysm:ignis": [2250, 70, 0],
	"cataclysm:endermaptera": [80, 20, 6],
	"cataclysm:deepling": [130, 20, 0],
	"cataclysm:deepling_brute": [300, 25, 8],
	"cataclysm:deepling_angler": [150, 20, 0],
	"cataclysm:deepling_priest": [225, 20, 0],
	"cataclysm:deepling_warlock": [225, 20, 0],
	"cataclysm:lionfish": [60, 10, 0],
	"cataclysm:coral_golem": [550, 55, 5],
	"cataclysm:coralssus": [800, 50, 5],
	"cataclysm:ignited_revenant": [400, 30, 2],
	"cataclysm:ignited_berserker": [325, 35, 8],
	"cataclysm:the_harbinger": [1950, 45, 2],
	"cataclysm:the_watcher": [125, 25, 5],
	"cataclysm:the_prowler": [800, 70, 0],
	"cataclysm:the_leviathan": [2000, 75, 0],
	"cataclysm:the_baby_leviathan": [600, 20, 5],
	"cataclysm:amethyst_crab": [1000, 65, 0],
	"cataclysm:ancient_ancient_remnant": [2000, 125, 0],
	"cataclysm:ancient_remnant": [2250, 125, 2],
	"cataclysm:modern_remnant": [750, 30, 5],
	"cataclysm:koboleton": [125, 15, 0],
	"cataclysm:kobolediator": [900, 70, 0],
	"cataclysm:wadjet": [750, 55, 5],
	"cataclysm:maledictus": [2100, 65, 0],
	"cataclysm:draugr": [140, 20, 3],
	"cataclysm:royal_draugr": [150, 25, 5],
	"cataclysm:elite_draugr": [160, 25, 3],
	"cataclysm:aptrgangr": [800, 90, 0],
	"cataclysm:hippocamtus": [425, 50, 5],
	"cataclysm:cindaria": [300, 35, 0],
	"cataclysm:clawdian": [1125, 80, 2],
	"cataclysm:scylla": [1950, 90, 2],
	"cataclysm:urchinkin": [60, 15, 0],
	"cataclysm:drowned_host": [100, 15, 2],
	"cataclysm:symbiocto": [80, 15, 0],
	"tide:tuna": [15, 15, 0],
	"tide:ocean_perch": [15, 15, 0],
	"tide:bluegill": [15, 15, 0],
	"tide:pike": [15, 15, 0],
	"tide:trout": [15, 15, 0],
	"tide:barracuda": [15, 15, 0],
	"tide:guppy": [15, 15, 0],
	"tide:clayfish": [15, 15, 0],
	"tide:catfish": [15, 15, 0],
	"tide:mackerel": [15, 15, 0],
	"tide:bass": [15, 15, 0],
	"tide:angelfish": [15, 15, 0],
	"tide:sailfish": [15, 15, 0],
	"tide:yellow_perch": [15, 15, 0],
	"tide:mint_carp": [15, 15, 0],
	"artifacts:mimic": [300, 25, 0],
	"netherexp:apparition": [120, 20, 0],
	"netherexp:wisp": [10, 15, 0],
	"netherexp:vessel": [120, 10, 0],
	"netherexp:ecto_slab": [5, 5, 3],
	"netherexp:banshee": [100, 30, 0],
	"netherexp:stampede": [400, 40, 0],
	"netherexp:carcass": [150, 25, 0],
	"netherexp:false_carcass": [100, 25, 0],
	"unusualend:ender_blob": [120, 20, 0],
	"unusualend:undead_enderling": [150, 20, 0],
	"unusualend:endstone_trapper": [500, 20, 3],
	"unusualend:ender_firefly": [200, 0, 0],
	"unusualend:endstone_golem": [2500, 40, 0],
	"unusualend:dragling": [75, 15, 0],
	"unusualend:bolok": [30, 5, 0],
	"unusualend:enderblob_queen": [2500, 50, 0],
	"unusualend:block_updater": [50, 15, 0],
	"unusualend:spunkler": [150, 0, 0],
	"unusualend:void_crack": [50, 0, 0],
	"unusualend:warped_balloon": [50, 0, 0],
	"unusualend:glub": [100, 5, 8],
	"unusualend:void_bomb": [400, 5, 0],
	"unusualend:enderbulb": [240, 20, 4],
	"unusualend:small_enderbulb": [60, 15, 0],
	"unusualend:bluk": [20, 5, 0],
	"unusualend:summoned_dragling": [100, 15, 1],
	"moresnifferflowers:bobling": [50, 15, 0],
	"ecologics:coconut_crab": [100, 10, 0],
	"ecologics:penguin": [75, 10, 0],
	"ecologics:squirrel": [50, 15, 0],
	"aquamirae:golden_moth": [15, 15, 0],
	"aquamirae:maw": [100, 20, 0],
	"aquamirae:anglerfish": [200, 30, 2],
	"aquamirae:maze_mother": [500, 25, 6],
	"aquamirae:captain_cornelia": [1000, 5, 6],
	"aquamirae:pillagers_patrol": [100, 15, 0],
	"aquamirae:tortured_soul": [150, 35, 4],
	"aquamirae:eel": [900, 40, 0],
	"aquamirae:maze_rose": [100, 15, 0],
	"aquamirae:poisoned_chakra": [100, 15, 0],
	"aquamirae:spinefish": [100, 15, 0],
	"aquamirae:luminous_jelly": [100, 15, 0],
	"ars_nouveau:ally_vex": [70, 20, 0],
	"ars_nouveau:bookwyrm": [30, 15, 0],
	"ars_nouveau:starbuncle": [100, 15, 0],
	"ars_nouveau:whirlisprig": [100, 15, 0],
	"ars_nouveau:wixie": [100, 15, 0],
	"ars_nouveau:wilden_hunter": [100, 20, 2],
	"ars_nouveau:summon_wolf": [40, 20, 0],
	"ars_nouveau:wilden_stalker": [75, 10, 0],
	"ars_nouveau:summon_horse": [265, 15, 0],
	"ars_nouveau:summon_skeleton": [100, 10, 0],
	"ars_nouveau:wilden_guardian": [125, 20, 2],
	"ars_nouveau:wilden_boss": [5000, 40, 6],
	"ars_nouveau:dummy": [0, 0, 0],
	"ars_nouveau:drygmy": [100, 15, 0],
	"ars_nouveau:familiar_starbuncle": [0, 0, 0],
	"ars_nouveau:familiar_wixie": [0, 0, 0],
	"ars_nouveau:familiar_bookwyrm": [0, 0, 0],
	"ars_nouveau:familiar_drygmy": [0, 0, 0],
	"ars_nouveau:familiar_whirlisprig": [0, 0, 0],
	"ars_nouveau:familiar_amethyst_golem": [0, 0, 0],
	"ars_nouveau:cascading_weald_walker": [300, 50, 0],
	"ars_nouveau:flourishing_weald_walker": [300, 50, 0],
	"ars_nouveau:blazing_weald_walker": [300, 50, 0],
	"ars_nouveau:vexing_weald_walker": [300, 50, 0],
	"ars_nouveau:amethyst_golem": [100, 15, 0],
	"ars_nouveau:gift_starby": [30, 15, 0],
	"ars_nouveau:animated_block": [20, 10, 0],
	"ars_nouveau:animated_head": [20, 10, 0],
	"ars_nouveau:lily": [200, 10, 0],
	"ars_elemental:siren_entity": [50, 15, 0],
	"ars_elemental:siren_familiar": [0, 0, 0],
	"ars_elemental:firenando_entity": [200, 15, 0],
	"ars_elemental:firenando_familiar": [0, 0, 0],
	"ars_elemental:fire_mage": [250, 5, 0],
	"ars_elemental:water_mage": [250, 5, 0],
	"ars_elemental:air_mage": [250, 5, 0],
	"ars_elemental:earth_mage": [250, 5, 0],
	"ars_elemental:flashing_weald_walker": [300, 50, 0],
	"ars_elemental:summon_skelehorse": [265, 15, 0],
	"ars_elemental:summon_camel": [265, 15, 0],
	"ars_elemental:summon_direwolf": [40, 20, 0],
	"ars_elemental:summon_wskeleton": [100, 10, 0],
	"ars_elemental:summon_dolphin": [50, 15, 0],
	"ars_elemental:summon_strider": [100, 15, 0],
	"ars_elemental:summon_vhex": [100, 40, 0],
	"crittersandcompanions:dragonfly": [20, 10, 0],
	"crittersandcompanions:dumbo_octopus": [70, 15, 0],
	"crittersandcompanions:ferret": [80, 15, 0],
	"crittersandcompanions:jumping_spider": [70, 40, 0],
	"crittersandcompanions:koi_fish": [40, 15, 0],
	"crittersandcompanions:leaf_insect": [30, 15, 0],
	"crittersandcompanions:otter": [80, 15, 0],
	"crittersandcompanions:red_panda": [90, 15, 0],
	"crittersandcompanions:sea_bunny": [40, 15, 0],
	"crittersandcompanions:shima_enaga": [50, 15, 0],
	"aether:phyg": [50, 15, 0],
	"aether:flying_cow": [50, 15, 0],
	"aether:sheepuff": [40, 15, 0],
	"aether:moa": [175, 25, 0],
	"aether:aerbunny": [30, 15, 0],
	"aether:aerwhale": [100, 15, 0],
	"aether:blue_swet": [5, 0, 0],
	"aether:golden_swet": [5, 0, 0],
	"aether:whirlwind": [50, 15, 0],
	"aether:evil_whirlwind": [50, 15, 0],
	"aether:aechor_plant": [75, 15, 0],
	"aether:cockatrice": [100, 15, 0],
	"aether:zephyr": [25, 15, 0],
	"aether:mimic": [200, 15, 0],
	"aether:sentry": [50, 10, 0],
	"aether:slider": [2000, 15, 0],
	"aether:valkyrie": [250, 50, 0],
	"aether:valkyrie_queen": [2500, 65, 0],
	"aether:fire_minion": [200, 75, 0],
	"aether:sun_spirit": [2500, 15, 0],
	"aether:cloud_minion": [5, 15, 0],
	"lost_aether_content:aerwhale_king": [2500, 75, 0],
	"aether_redux:vanilla_swet": [5, 0, 0],
	"aether_redux:shimmercow": [125, 15, 0],
	"aether_redux:mykapod": [30, 15, 0],
	"aether_redux:blightbunny": [50, 15, 0],
	"aether_redux:cockatrice_spawner": [100, 15, 0],
	"aether_redux:blightbunny_spawner": [100, 15, 0],
	"naturalist:alligator": [150, 30, 0],
	"naturalist:bass": [15, 15, 0],
	"naturalist:bear": [150, 30, 0],
	"naturalist:bluejay": [30, 15, 0],
	"naturalist:boar": [70, 5, 0],
	"naturalist:butterfly": [50, 15, 0],
	"naturalist:canary": [30, 15, 0],
	"naturalist:cardinal": [30, 15, 0],
	"naturalist:caterpillar": [25, 15, 0],
	"naturalist:catfish": [30, 5, 0],
	"naturalist:coral_snake": [50, 30, 0],
	"naturalist:deer": [40, 15, 0],
	"naturalist:dragonfly": [30, 15, 0],
	"naturalist:duck": [20, 15, 0],
	"naturalist:elephant": [400, 50, 0],
	"naturalist:firefly": [30, 15, 0],
	"naturalist:finch": [30, 15, 0],
	"naturalist:giraffe": [175, 15, 0],
	"naturalist:hippo": [200, 30, 0],
	"naturalist:lion": [120, 30, 0],
	"naturalist:lizard": [40, 10, 0],
	"naturalist:lizard_tail": [30, 15, 0],
	"naturalist:moose": [200, 30, 0],
	"naturalist:rattlesnake": [50, 30, 0],
	"naturalist:rhino": [200, 30, 0],
	"naturalist:robin": [30, 15, 0],
	"naturalist:snail": [10, 15, 0],
	"naturalist:snake": [50, 30, 0],
	"naturalist:sparrow": [30, 15, 0],
	"naturalist:tortoise": [100, 10, 0],
	"naturalist:vulture": [50, 20, 0],
	"naturalist:zebra": [265, 15, 0],
	"dummmmmmy:target_dummy": [1000, 0, 0],
	"witherstormmod:wither_storm": [2000, 15, 8],
	"witherstormmod:wither_storm_segment": [20000, 10, 6],
	"witherstormmod:sickened_zombie": [120, 15, 2],
	"witherstormmod:sickened_skeleton": [120, 10, 0],
	"witherstormmod:sickened_spider": [100, 15, 0],
	"witherstormmod:sickened_creeper": [130, 10, 0],
	"witherstormmod:command_block": [320, 15, 2],
	"witherstormmod:withered_symbiont": [300, 80, 0],
	"witherstormmod:wither_storm_head": [300, 10, 8],
	"witherstormmod:tentacle": [400, 10, 2],
	"witherstormmod:sickened_villager": [120, 15, 2],
	"witherstormmod:sickened_phantom": [100, 15, 0],
	"witherstormmod:sickened_chicken": [80, 10, 0],
	"witherstormmod:sickened_parrot": [80, 10, 0],
	"witherstormmod:sickened_wolf": [40, 20, 0],
	"witherstormmod:sickened_cat": [100, 20, 0],
	"witherstormmod:sickened_cow": [125, 10, 0],
	"witherstormmod:sickened_pig": [100, 10, 0],
	"witherstormmod:sickened_mushroom_cow": [130, 10, 0],
	"witherstormmod:sickened_bee": [75, 10, 0],
	"witherstormmod:sickened_pillager": [150, 30, 0],
	"witherstormmod:sickened_vindicator": [150, 30, 0],
	"witherstormmod:sickened_iron_golem": [300, 50, 0],
	"witherstormmod:sickened_snow_golem": [40, 10, 0],
	"witherstormmod:tainted_slime": [0, 0, 0],
	"create:package": [25, 15, 0],
	"netherdepthsupgrade:lava_pufferfish": [15, 15, 0],
	"netherdepthsupgrade:obsidianfish": [15, 15, 5],
	"netherdepthsupgrade:searing_cod": [15, 15, 0],
	"netherdepthsupgrade:bonefish": [15, 15, 0],
	"netherdepthsupgrade:wither_bonefish": [15, 15, 0],
	"netherdepthsupgrade:blazefish": [15, 15, 0],
	"netherdepthsupgrade:magmacubefish": [15, 15, 0],
	"netherdepthsupgrade:glowdine": [15, 15, 0],
	"netherdepthsupgrade:soulsucker": [15, 15, 0],
	"netherdepthsupgrade:fortressgrouper": [100, 50, 0],
	"netherdepthsupgrade:eyeball_fish": [40, 15, 0],
	"upgrade_aquatic:nautilus": [90, 15, 0],
	"upgrade_aquatic:pike": [60, 15, 0],
	"upgrade_aquatic:perch": [15, 15, 0],
	"upgrade_aquatic:lionfish": [40, 15, 0],
	"upgrade_aquatic:thrasher": [250, 25, 8],
	"upgrade_aquatic:great_thrasher": [625, 40, 6],
	"upgrade_aquatic:flare": [100, 10, 0],
	"upgrade_aquatic:goose": [30, 15, 0],
	"upgrade_aquatic:box_jellyfish": [0, 25, 0],
	"upgrade_aquatic:cassiopea_jellyfish": [0, 5, 0],
	"upgrade_aquatic:immortal_jellyfish": [35, 5, 0],
	"buzzier_bees:grizzly_bear": [150, 30, 0],
	"buzzier_bees:moobloom": [50, 15, 0],
	"alexscaves:teletor": [90, 10, 0],
	"alexscaves:magnetron": [150, 10, 6],
	"alexscaves:boundroid": [100, 25, 0],
	"alexscaves:boundroid_winch": [100, 25, 0],
	"alexscaves:ferrouslime": [50, 10, 0],
	"alexscaves:notor": [30, 10, 0],
	"alexscaves:subterranodon": [100, 10, 0],
	"alexscaves:vallumraptor": [140, 15, 0],
	"alexscaves:grottoceratops": [250, 50, 8],
	"alexscaves:trilocaris": [50, 5, 0],
	"alexscaves:tremorsaurus": [750, 70, 8],
	"alexscaves:relicheirus": [600, 60, 0],
	"alexscaves:luxtructosaurus": [3000, 60, 0],
	"alexscaves:atlatitan": [2000, 40, 0],
	"alexscaves:nucleeper": [200, 10, 4],
	"alexscaves:radgill": [40, 10, 0],
	"alexscaves:brainiac": [200, 25, 8],
	"alexscaves:gammaroach": [70, 10, 0],
	"alexscaves:raycat": [120, 5, 0],
	"alexscaves:tremorzilla": [2500, 150, 0],
	"alexscaves:lanternfish": [10, 10, 0],
	"alexscaves:sea_pig": [40, 10, 0],
	"alexscaves:hullbreaker": [2000, 80, 0],
	"alexscaves:gossamer_worm": [50, 10, 0],
	"alexscaves:tripodfish": [40, 10, 0],
	"alexscaves:deep_one": [150, 15, 0],
	"alexscaves:deep_one_knight": [300, 25, 0],
	"alexscaves:deep_one_mage": [400, 20, 0],
	"alexscaves:mine_guardian": [100, 5, 0],
	"alexscaves:gloomoth": [20, 10, 0],
	"alexscaves:underzealot": [100, 20, 0],
	"alexscaves:watcher": [150, 20, 0],
	"alexscaves:corrodent": [80, 15, 2],
	"alexscaves:vesper": [80, 15, 0],
	"alexscaves:forsaken": [1250, 50, 0],
	"alexscaves:sweetish_fish": [20, 10, 0],
	"alexscaves:caniac": [190, 10, 0],
	"alexscaves:gumbeeper": [70, 20, 4],
	"alexscaves:candicorn": [150, 30, 0],
	"alexscaves:gum_worm": [750, 45, 0],
	"alexscaves:caramel_cube": [20, 10, 0],
	"alexscaves:gummy_bear": [180, 20, 0],
	"alexscaves:licowitch": [200, 15, 0],
	"alexscaves:gingerbread_man": [50, 10, 0],
	"quark:crab": [100, 15, 3],
	"quark:forgotten": [300, 10, 0],
	"quark:foxhound": [40, 20, 0],
	"quark:shiba": [40, 20, 0],
	"quark:stoneling": [40, 15, 0],
	"quark:toretoise": [300, 15, 0],
	"quark:wraith": [100, 15, 0],
	"supplementaries:hat_stand": [100, 15, 0],
	"supplementaries:red_merchant": [100, 15, 0],
	"rediscovered:fish": [15, 15, 0],
	"rediscovered:pigman": [100, 15, 0],
	"rediscovered:melee_pigman": [200, 10, 0],
	"rediscovered:ranged_pigman": [100, 10, 0],
	"rediscovered:zombie_pigman": [100, 25, 2],
	"rediscovered:steve": [250, 10, 0],
	"rediscovered:rana": [250, 10, 0],
	"rediscovered:black_steve": [250, 10, 0],
	"rediscovered:beast_boy": [250, 10, 0],
	"rediscovered:red_dragon": [1500, 15, 0],
	"rediscovered:red_dragon_offspring": [500, 15, 0],
	"rediscovered:dragon_pylon": [150, 15, 0],
	"rediscovered:scarecrow": [50, 15, 0]
}

global.mobs = []

EntityJSEvents.attributes(event => {

	global.mobs = event.getAllTypes();

	Object.entries(global.hpModifications).forEach(([entityId, values]) => {
		event.modify(entityId, attributes => {
			if (values[0] !== null) {
				attributes.add('generic.max_health', values[0]);
			}
			if (values[1] !== undefined && values[1] !== null) {
				attributes.add('generic.attack_damage', values[1]);
			}
			if (values[2] !== undefined && values[2] !== null) {
				attributes.add('generic.armor', values[2]);
			}
		})
	});

	event.modify('player', attributes => {
		attributes.add('generic.max_health', 100);
		attributes.add('generic.attack_damage', 3)
	})
})

EntityJSEvents.modifyEntity(event => {
	event.modify('born_in_chaos_v1:maggot', entity => {
		entity.canAttack(context => false)
		entity.canCollideWith(context => false)
	})
})
