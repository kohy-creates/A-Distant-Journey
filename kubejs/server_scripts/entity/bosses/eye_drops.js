...
EntityEvents.death(event => {
	...
	if (Object.keys(global.eyeDrops).includes(entityType)) {
		const eye = global.eyeDrops[entityType];
		...
		if (global.ifRandomChance(chance)) {
			server.runCommandSilent(
				`/execute in ${entity.level.dimension} positioned ${entity.x} ${entity.y + (entity.eyeHeight / 2)} ${entity.z} ${(entity.getType() === 'rediscovered:red_dragon') ? 'at @p positioned ~ ~12 ~ ' : ''}run summon marker ~ ~ ~ {Tags:["adj.eye_marker", "adj.eye_of_${eye}"]}`
			);
