ServerEvents.recipes((event) => {
	...
	const removeRecipeByID = [
		...
		/(?!.*sapling)ars_nouveau:manipulation_essence_to_/,
		...
		'mythicmetals:smelting/prometheum_ingot_from_ores',
		'mythicmetals:smelting/prometheum_ingot_from_raw_ore',
	...
	event.smoking('dried_kelp', 'hybrid-aquatic:sea_lettuce', 0.2, 200).id('adj:dried_kelp_from_sea_lettuce');
	...
	sewingRecipe('white_wool', 'aether:white_cape');
	sewingRecipe('red_wool', 'aether:red_cape');
	...
	event.shaped(
		Item.of('recovery_compass').enchant('sortilege:soulbound', 1),
		[
			' E ',
			'ECE',
			' E '
	...
	/**
	 * Registers both a Terra Plate recipe as well as a Gaia Plate equivalent.
	 * Gaia Plate applies a 40% discount on Mana consumption.
	 * 
	 * @param {Internal.InputItem_} inputs - List of items or tags (prefix with # for tags)
	 * @param {Internal.OutputItem_} output - The resulting item ID
	 * @param {number} mana - Mana cost
	...
	function alloyForgeRecipe(inputs, output, minTier, fuelPerTick, tierOverrides) {
		...
		event.custom(recipe)
			.id(`adj:alloying/${flattenedID((Array.isArray(output)) ? output[0] :
				((output.includes(' ')) ? output.split(' ')[1] : output))}_from_${flattenedID(inputs[0][0])}`);
	}
	...
	ironArmorRecipe(['SIS', 'I I'], 'iron_helmet');
	ironArmorRecipe(['I I', 'SIS', 'SIS'], 'iron_chestplate');
	ironArmorRecipe(['SIS', 'I I', 'I I'], 'iron_leggings');
	ironArmorRecipe(['I I', 'S S'], 'iron_boots');
	...
	diamondItems.forEach((item) => {
		event.remove({ output: `diamond_${item}` });
		event.smithing(
			`diamond_${item}`,
			'kubejs:diamond_upgrade',
	...
	furnaceRecipe('cobblestone', 'furnace');
	furnaceRecipe('cobbled_deepslate', 'quark:deepslate_furnace');
	furnaceRecipe('blackstone', 'quark:blackstone_furnace');
	...
	Item.getTypeList().forEach(id => {
		if (!global.isItemDisabled(id) && id.includes('_sherd')) {
			event.shapeless(
				Item.of(id, 2),
				[
					id,
	...
	ectoplasmTransform('magma_block', 'netherexp:soul_magma_block');
	ectoplasmTransform('soul_sand', 'netherexp:ecto_soul_sand');
	ectoplasmTransform('blackstone', 'netherexp:soul_slate');
	ectoplasmTransform('torchflower', 'netherexp:soul_torchflower');
	ectoplasmTransform('coal', 'netherexp:fossil_fuel');
	ectoplasmTransform('rib_armor_trim_smithing_template', 'netherexp:valor_armor_trim_smithing_template');
	ectoplasmTransform('shroomlight', 'netherexp:shroomnight');
	...
	function gearRecipe(amount, material) {
		...
		).id(`adj:gear_from_${flattenedID(material)}`);
	}

	gearRecipe(2, 'iron_nugget');
	gearRecipe(1, 'create:zinc_nugget');
	...
	event.replaceInput({ input: 'popped_chorus_fruit', type: 'ars_nouveau:enchanting_apparatus' },
		"popped_chorus_fruit",
		"waystones:warp_dust"
	);
	...
	event.forEachRecipe({ type: 'crafting_shaped', output: /cataclysm\:.*eye/ }, recipe => {
		...
		originalIngredients.forEach(ingredient => {
			let isAir = ingredient.getItemIds().some(i => i == 'air');
	...
	).id('adj:awakened_pearls/mech_eye');
	...
	// Ars Nouveau rework and re-progression
	/**
	 * @type {Special.RecipeId[]}
	 */
	const removeArsRecipes = [
	...
	event.shaped(
		...
		{
			M: 'ars_nouveau:sourcestone',
			A: 'amethyst_shard',
			L: 'botania:livingwood_twig'
		}
	).id('adj:arcane_pedestal');
	...
	function elementalUpgradeRecipe(element, essence) {
		let armorPieces = ['boots', 'robes', 'leggings', 'hat'];
		let armorTags = ['ars_nouveau:boot', 'ars_nouveau:robe', 'ars_nouveau:legs', 'ars_nouveau:hood'];
		let i = 0;
	...
	event.recipes.botania.mana_infusion('ars_nouveau:source_gem', ['amethyst_shard', 'lapis_lazuli'])
		.mana(manaGemCost)
		.id('adj:mana_gem');

	event.recipes.botania.mana_infusion('ars_nouveau:source_gem_block', 'amethyst_block')
		.mana(manaGemCost * 4)
		.id('adj:mana_gem_block');

	event.recipes.botania.mana_infusion('ars_nouveau:source_gem_block', 'lapis_block')
		.mana(manaGemCost * 9)
	...
	// Glyphs rework
	const glyphsMap = {
		'ars_nouveau:glyph_amplify': ['blaze_powder', 'blaze_powder', 'botania:manasteel_ingot'],
		...
		'ars_nouveau:glyph_aoe': ['quark:gunpowder_sack', 'quark:gunpowder_sack', 'hybrid-aquatic:sulfur', 'hybrid-aquatic:sulfur', 'hybrid-aquatic:sulfur', 'bone_meal'],
		...
		'ars_nouveau:glyph_explosion': ['botania:rune_fire', 'tnt', 'hybrid-aquatic:sulfur', 'hybrid-aquatic:sulfur', 'mythicmetals:morkite_block', 'mythicmetals:morkite_block',],
		...
		'ars_nouveau:glyph_burst': ['botania:rune_summer', 'botania:rune_wrath', 'hybrid-aquatic:sulfur', 'hybrid-aquatic:sulfur', 'firework_star'],
		...
		'ars_nouveau:glyph_dispel': ['botania:rune_mana', 'nether_wart', 'milk_bucket', 'milk_bucket',],
		'ars_nouveau:glyph_duration_down': ['clock', 'glowstone_dust', 'glowstone_dust'],
		'ars_nouveau:glyph_evaporate': ['botania:rune_summer', 'sponge', 'sponge', 'sponge'],
		'ars_nouveau:glyph_exchange': ['botanicadds:rune_tp', 'botania:elementium_ingot', 'waystones:warp_dust', 'waystones:warp_dust', 'waystones:warp_dust'],
		'ars_nouveau:glyph_fangs': ['kubejs:rune_life', 'emerald_block', 'emerald_block', 'twilightforest:charm_of_life_2'],
		'ars_nouveau:glyph_fell': ['botania:rune_earth', '#c:axes', '#c:axes', '#c:axes'],
		'ars_nouveau:glyph_flare': ['botania:rune_fire', 'flint_and_steel', 'fire_charge', 'magma_block', 'magma_block'],
		'ars_nouveau:glyph_fortune': ['kubejs:decayed_clover'],
		...
		'ars_nouveau:glyph_gravity': ['botanicadds:rune_energy', 'botania:rune_mana', 'quark:gravisand', 'quark:gravisand', 'quark:gravisand',],
		...
		'ars_nouveau:reset': ['target', 'target', 'botania:mana_pearl'],
		...
		'ars_nouveau:wololo': ['red_dye', 'red_dye', 'blue_dye', 'blue_dye', 'botania:rune_mana']
	};
	...
	// Essences and essence transforms
	function essenceTransform(essence, material, result, amount) {
		if (!amount) amount = 8;
		let inputs = [
			essence,
		]
		...
		event.shapeless(
			Item.of(result, amount),
			inputs
		).id(`adj:essence/${flattenedID(result)}_from_${flattenedID(material)}_and_${flattenedID(essence)}`)
	}

	essenceTransform('ars_nouveau:fire_essence', '#forge:stone', 'magma_block');
	essenceTransform('ars_nouveau:manipulation_essence', 'diorite', 'granite')
	essenceTransform('ars_nouveau:manipulation_essence', 'andesite', 'diorite')
	essenceTransform('ars_nouveau:manipulation_essence', 'granite', 'andesite')
	essenceTransform('ars_nouveau:manipulation_essence', 'tuff', 'calcite')
	essenceTransform('ars_nouveau:manipulation_essence', 'calcite', 'deepslate')
	essenceTransform('ars_nouveau:manipulation_essence', 'deepslate', 'tuff')
	...
	event.forEachRecipe([{ type: 'smelting', output: /nugget/ }, { type: 'blasting', output: /nugget/ }], recipe => {
		...
		if (output.getId().includes('nugget') && !nuggetsBlacklist.includes(output.getId())) {
			event.remove({ id: recipe.getId() });
			if (recipe.getType().toString() == 'blasting') {
				event.blasting(
					Item.of(output, 5),
					input[0],
					0.1,
	...
	event.recipes.ars_nouveau.enchanting_apparatus(
		...
		"compass",
		Item.of('compass', '{LodestoneDimension:"overworld",LodestonePos:{X:3440,Y:64,Z:3200},LodestoneTracked:0b,display:{Name:\'{"text":"Command Block Compass","italic":false}\'},"quark:compass_calculated":1b,"quark:compass_in_nether":0b}'),
		0
	...
	event.recipes.create.sequenced_assembly([
		...
		event.recipes.create.filling(inter, [inter, Fluid.of('lava', 1000)]),
	...
	function FSUpgradeRecipe(input, result) {
		event.recipes.create.sequenced_assembly([
			...
			event.recipes.create.filling(inter, [inter, Fluid.of('lava', 250)]),
		...
		function clearResult(axis) {
			...
			return [
				...
				(axis == "x") ?
					{
						"type": "execute",
						"command": `fill ~1 ~1 ~ ~-1 ~ ~ air replace ${block}`,
						"hide": true
					} :
					{
						"type": "execute",
				...
				{
					"type": "execute",
					"command": "playsound block.anvil.use block @a ~ ~ ~",
					"hide": true
				}
			]
		}
	...
	event.forEachRecipe({ output: /^(?=.*fence)(?!.*gate).*/, type: 'crafting_shaped' }, recipe => {
		...
		event.shaped(
				...
				],
			{
				...
				S: (ingredients[Math.abs(slotToUse - 1)].getItemIds().toArray().includes('stick')
	...
	event.forEachRecipe({ output: /fence_gate/, type: 'crafting_shaped' }, recipe => {
		...
		event.shaped(
			...
			{
				P: ingredients[1],
				S: (ingredients[0].getItemIds().toArray().includes('stick'))
	...
	event.forEachRecipe({ output: /^(?=.*_sign)(?!.*hanging)(?!.*post).*/, type: 'crafting_shaped' }, recipe => {
		...
		event.shaped(
			...
			],
			{
				...
				E: ingredients[4],
				F: ingredients[5],
				G: (ingredients[7].getItemIds().toArray().includes('stick')) ? 'stick' : ingredients[7],
			}
		).id(recipe.getId())
	})
	...
	function rune(name) {
		...
		for (let supplier of options) {
			...
			if (Item.exists(id)) return id;
		}
		return null;
	}
	const manaAmounts = [5200, 8000, 12000];
	const runeCraftingMap = {
		'water': { tier: 1, items: ['botania:mana_powder', 'ars_nouveau:water_essence'] },
		'fire': { tier: 1, items: ['botania:mana_powder', 'ars_nouveau:fire_essence'] },
		'air': { tier: 1, items: ['botania:mana_powder', 'ars_nouveau:air_essence'] },
		'earth': { tier: 1, items: ['botania:mana_powder', 'ars_nouveau:earth_essence'] },
		'mana': { tier: 1, items: ['botania:mana_powder', 'ars_nouveau:abjuration_essence'] },
		'spring': { tier: 2, items: [rune('water'), rune('fire'), 'botania:mana_powder', 'ars_nouveau:source_gem'] },
		'summer': { tier: 2, items: [rune('air'), rune('earth'), 'botania:mana_powder', 'ars_nouveau:source_gem'] },
		'autumn': { tier: 2, items: [rune('fire'), rune('air'), 'botania:mana_powder', 'ars_nouveau:source_gem'] },
		'winter': { tier: 2, items: [rune('earth'), rune('water'), 'botania:mana_powder', 'ars_nouveau:source_gem'] },
		'tp': { tier: 1, items: ['botania:mana_powder', 'ars_nouveau:manipulation_essence'] },
		'energy': { tier: 1, items: ['botania:mana_powder', 'ars_nouveau:conjuration_essence'] },
		'life': { tier: 1, items: ['botania:mana_powder', 'ars_elemental:anima_essence'] }
	};
	...
	const essenceToRuneMap = {
		...
		'ars_nouveau:conjuration_essence': 'botanicadds:rune_energy',
	};
	...
	Item.getTypeList().forEach(id => {
		if (
			!global.isItemDisabled(id)
			&& id.includes('_leaves')
			&& id !== 'botania:horn_leaves'
		) {
	...
	Item.getTypeList().forEach(id => {
		if (global.isItemDisabled(id)) return;
		const maxDamage = Item.of(id).maxDamage;
		if (maxDamage > 0) {
			event.custom({
				...
				repairTime: Math.ceil((1.4 * Math.sqrt(maxDamage)) * 20)
	...
	event.recipes.summoningrituals.altar('born_in_chaos_v1:intoxicating_decoction')
		.itemOutput('born_in_chaos_v1:transmuting_elixir')
		.input(
		...
		.recipeTime(toSeconds(10))
		.id('adj:transmuting_elixir');

	event.recipes.summoningrituals.altar('botania:tiny_planet')
		.itemOutput('terra_curio:gravity_globe')
		.input(
	...
	event.recipes.summoningrituals.altar('mcdw:crossbow_heavy_crossbow')
		.itemOutput('kubejs:elsa_crossbow')
		.input(
		...
		.recipeTime(toSeconds(10))
		.id('adj:elsa_bloodstone_crossbow');
	...
	event.replaceInput({ id: 'tinted_glass' },
		'amethyst_shard',
		[
			'amethyst_shard',
			'evilcraft:dark_gem'
	...
	const missingChests = [
		'twilightforest:canopy', 'twilightforest:twilight_oak', 'twilightforest:dark',
		'upgrade_aquatic:river', 'upgrade_aquatic:driftwood'
	];
	missingChests.forEach(t => {
	...
	).id('adj:rainbow_oak_sapling');

	event.recipes.botania.mana_infusion('twilightforest:hollow_oak_sapling', 'twilightforest:twilight_oak_sapling').mana(250000).id('adj:robust_twilight_oak_sapling');

	function fieryItemRecipe(output, shape) {
		let ingr = {};
		shape.forEach(l => l.split('').forEach(char => {
			const a = {
	...
	fieryItemRecipe('twilightforest:fiery_helmet', [
		'IOI',
		'O O'
	]);
	fieryItemRecipe('twilightforest:fiery_chestplate', [
		'I I',
		'ORO',
		'OIO'
	]);
	fieryItemRecipe('twilightforest:fiery_leggings', [
		'IRI',
		'O O',
		'O O'
	]);
	fieryItemRecipe('twilightforest:fiery_boots', [
		'O O',
		'I I'
	]);
	fieryItemRecipe('twilightforest:fiery_sword', [
		' I ',
		'OIO',
		' B '
	]);
	fieryItemRecipe('twilightforest:fiery_pickaxe', [
		'IRI',
		' B ',
		' B '
	]);
	...
	alloyForgeRecipe(
		[
			['#c:raw_palladium_ores', 1],
			['#c:raw_gold_ores', 1],
			['twilightforest:fiery_blood', 2]
		],
		['twilightforest:fiery_ingot', 3],
		2,
		15,
		[
			['4+', 'output', 4],
	...
	/**
	 * 
	 * @param {Internal.FluidType_} fluid 
	 * @param {Internal.Item_} item 
	 * @param {Internal.Item_} containerItem 
	 */
	...
	/**
	 * 
	 * @param {Internal.FluidType_} basefluid 
	 * @param {Internal.Item_[]} ingredients 
	 * @param {Internal.FluidType_} resultFluid 
	 * @param {Internal.Item_} resultItem 
	 * @param {Internal.Item_} containerItem
	 * Defaults to `vinery:wine_bottle` 
	 * @param {number} temperature 
	 * Number ranges from 1 - 5, defaults to 3
	 * @param {number} experience
	 * Defaults to 1.0 
	 * @param {number} fermentingtime
	 * Defaults to 9600, which is 480 seconds (8 minutes) 
	 */
	...
	fermentingRecipe('kubejs:white_taiga_grapejuice', ['twilightforest:aurora_block', 'neapolitan:ice_cubes', 'neapolitan:ice_cubes'], 'kubejs:eiswein', 'vinery:eiswein');
	...
	fermentingRecipe('honey', ['ice', 'sugar'], 'kubejs:coconut_cocktail', 'beachparty:coconut_cocktail', 'beachparty:coconut_open', 1, 1.0, 4800);
	fermentingRecipe('brewinandchewin:red_rum', ['neapolitan:ice_cubes', 'sweet_berries', 'sugar'], 'kubejs:sweetberry_cocktail', 'beachparty:sweetberries_cocktail', 'glass_bottle', 1, 1.0, 4800);
	fermentingRecipe('milk', ['neapolitan:ice_cubes', 'cocoa_beans', 'sugar'], 'kubejs:cocoa_cocktail', 'beachparty:cocoa_cocktail', 'glass_bottle', 1, 1.0, 4800);
	fermentingRecipe('milk', ['neapolitan:ice_cubes', 'farmersdelight:pumpkin_slice', 'farmersdelight:pumpkin_slice', 'sugar'], 'kubejs:pumpkin_cocktail', 'beachparty:pumpkin_cocktail', 'glass_bottle', 1, 1.0, 4800);
	fermentingRecipe('brewinandchewin:beer', ['neapolitan:ice_cubes', 'melon_slice', 'neapolitan:strawberries', 'sugar'], 'kubejs:melon_cocktail', 'beachparty:melon_cocktail', 'glass_bottle', 1, 1.0, 4800);
	fermentingRecipe('honey', ['neapolitan:ice_cubes', 'farmersdelight:milk_bottle', 'farmersdelight:milk_bottle'], 'kubejs:honey_cocktail', 'beachparty:honey_cocktail', 'glass_bottle', 1, 1.0, 4800);
	...
	event.shaped(
		'1x create_connected:copycat_box',
		[
			'CC ',
			'C C',
			' CC'
	...
	event.shaped(
		'3x create_connected:copycat_fence_gate',
		[
			'SCS',
			'SCS'
		],
	...
	// Sulfur and Cinnabar
	event.shaped('kubejs:polished_cinnabar_pressure_plate', 'AA', { A: 'kubejs:polished_cinnabar' }).id('adj:polished_cinnabar_pressure_plate');
	event.shaped('kubejs:polished_sulfur_pressure_plate', 'AA', { A: 'kubejs:polished_sulfur' }).id('adj:polished_sulfur_pressure_plate');
	...
	Color.DYE.forEach(c => {
		...
		).id(`adj:${c}_stone_bricks`);
		blockSet(`${c}_stone_bricks`);
	...
	event.recipes.create.milling(['hybrid-aquatic:sulfur', Item.of('hybrid-aquatic:sulfur').withChance(0.5)], 'kubejs:sulfur', 200).id('adj:crushed_sulfur');
	...
	function portalGunRecipe(dyes, variant) {
		...
		event.recipes.create.sequenced_assembly(
			...
			[
				...
				event.recipes.create.pressing(inter, inter),
				event.recipes.create.deploying(inter, [inter, Item.of('kubejs:manasteel_sheet')]),
				event.recipes.create.deploying(inter, [inter, Item.of('iron_block')]),
				event.recipes.create.pressing(inter, inter),
				event.recipes.create.filling(inter, [inter, Fluid.of('kubejs:dimensional_juice', 500)]),
			]
		).transitionalItem(inter).loops(8).id(`adj:portal_gun${variant ? `_${variant}` : ''}`);
	}
	...
	gaiaPlateRecipe(
		...
		Item.of('mythicmetals:prometheum_ingot', 5),
		500000
	);
	...
	// Rework Neptune, Phoenix and Obsidian armor
	function phoenixGearRecipe(input, output, addObsidianRecipeVariant, baseObsidianItem) {
		event.recipes.ars_nouveau.enchanting_apparatus(
			[
				'mythicmetals:star_platinum',
				'mythicmetals:midas_gold_block',
			...
			1000,
			true
		).id(`adj:phoenix_gear/${flattenedID(output)}`);

		if (addObsidianRecipeVariant) itemsOnGround(
			[baseObsidianItem],
			output,
			`adj:phoenix_gear/${flattenedID(output)}_from_obsidian_armor`,
			'ars_nouveau:magic_fire',
			'Toss the armor piece into Magic Fire\nto make it burn bright again');
	}

	phoenixGearRecipe('mythicmetals:palladium_helmet', 'aether:phoenix_helmet', true, 'aether:obsidian_helmet');
	phoenixGearRecipe('mythicmetals:palladium_chestplate', 'aether:phoenix_chestplate', true, 'aether:obsidian_chestplate');
	phoenixGearRecipe('mythicmetals:palladium_leggings', 'aether:phoenix_leggings', true, 'aether:obsidian_leggings');
	phoenixGearRecipe('mythicmetals:palladium_boots', 'aether:phoenix_boots', true, 'aether:obsidian_boots');

	phoenixGearRecipe('mythicmetals:palladium_sword', 'lost_aether_content:phoenix_sword');
	phoenixGearRecipe('mythicmetals:palladium_axe', 'lost_aether_content:phoenix_axe');
	phoenixGearRecipe('mythicmetals:palladium_pickaxe', 'lost_aether_content:phoenix_pickaxe');
	phoenixGearRecipe('mythicmetals:palladium_shovel', 'lost_aether_content:phoenix_shovel');
	phoenixGearRecipe('mythicmetals:palladium_hoe', 'lost_aether_content:phoenix_hoe');

	function neptuneArmor(type) {
		event.smithing(`aether:neptune_${type}`, 'mythicmetals:adamantite_ingot', `mythicmetals:tidesinger_${type}`, '#adj:tidesinger_upgrade_coral').id(`adj:neptune_armor/${type}`);
	}
	Array.from(['helmet', 'chestplate', 'leggings', 'boots']).forEach(type => neptuneArmor(type));

	// A  bit more universal Cakes
	 ...
	 * @type {Special.RecipeId[]}
	 */
	const removeACRecipes = [
		'alexscaves:ominous_catalyst'
	];
	removeACRecipes.forEach(recipe => {
		event.remove({ id: recipe });
	});

	alloyForgeRecipe(
		[
			['alexscaves:amber_curiosity', 1],
			['alexscaves:heavy_bone', 10]
		],
		['alexscaves:ominous_catalyst', 1],
		3,
		20,
		[]
	);
